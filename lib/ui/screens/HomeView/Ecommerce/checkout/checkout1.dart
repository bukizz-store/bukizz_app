...
class _Checkout1State extends State<Checkout1> {
    ...
    await FirebaseDatabase.instance
      ...
      if (snapshot.value != null) {
        if (snapshot.value.toString().contains(
            context.read<OrderViewRespository>().getUserAddress.pinCode)) {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const Checkout2()),
          );
        } else {
          AppConstants.showSnackBarTop(context, 'Delivery unavailable here',
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<UpdateAddressRepository>(
      builder: (context, value, child) {
        var address = value.address.toString();
        var alternateAddress = value.alternateAddress.toString();
        return Scaffold(
          appBar: AppBar(
          ...
          body: SingleChildScrollView(
            child: Column(
              children: [
                ...
                CheckoutProcessWidget(currentStep: 1),
                                  ...
                                  Radio<String>(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        ...
                                        context
                                            .read<OrderViewRespository>()
                                            .setUserAddress(context
                                                .read<UpdateAddressRepository>()
                                                .address);
                                      });
                                    },
                                  ),
